...
if __name__ == "__main__":
    ...
    for caption in train_captions:
        _from = os.path.join(root, caption["file_name"].replace("/images/", ""))
        _to = os.path.join(
            "./diffusion/train", caption["file_name"].replace("/images/", "")
        )
    ...
    for caption in valid_captions:
        _from = os.path.join(root, caption["file_name"].replace("/images/", ""))
        _to = os.path.join(
            "./diffusion/validation", caption["file_name"].replace("/images/", "")
        )
        shutil.move(_from, _to)

    with open("./diffusion/train/metadata.jsonl", "w") as f:
        for caption in train_captions:
            caption["file_name"] = caption["file_name"].replace("/images/", "")
